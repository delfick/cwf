...
def configure(settings, package, *parts, **kwargs):
    """
        Configure a site to exist in the package specified with the parts specified.
        This will inject into package.models and package.urls using the Parts object created
    ...
    # get the urls
    include_defaults = kwargs.get("include_default_urls", False)
    urls = lambda : config.urls(include_defaults=include_defaults)

    # Helper to determine names to inject values into
    prefix=kwargs.get("prefix", None)
    def names_for(name):
        names = ["%s.%s" % (package, name)]
        if prefix:
            names.append("%s.%s" % (prefix, names[0]))
        return names

    # Inject urls and models
    inject(urls, names_for('urls'))
    inject(models, names_for('models'))
    
    # Load all the admin configurations
    config.load_admin()
